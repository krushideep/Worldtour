...
type Decision={step:number;from:Capital;selected:Capital;candidates:{city:Capital;score:number;confidence?:number}[];distanceKm:number;confidence?:number;oracleIso2?:string;regretKm?:number;latencyMs?:number;strategy?:string};
...
async function jevDecide(step:number,current:Capital,cs:Capital[],rem:Capital[],settings:Settings,mode:JEVMode,matrix:Record<string,Record<string,number>>):Promise<{ranked:{city:Capital;confidence:number}[];mode:"demo"|"live";latencyMs?:number;strategy?:string}>{
...
async function copilotDecide(step:number,current:Capital,cs:Capital[],rem:Capital[],model:string):Promise<{selected:Capital;latencyMs:number}>{
  const started=performance.now();
  ...
  return{selected,latencyMs:data.latencyMs??(performance.now()-started)};
}
async function copilotEngine(c:Capital[],start:Capital,model:string,onStep?:(d:Decision,routeSoFar:Capital[])=>void){
  ...
  while(rem.length){
    ...
    const{selected,latencyMs}=await copilotDecide(route.length,cur,cs,rem,model);
    const pick=selected;
    const d:Decision={step:route.length,from:cur,selected:pick,candidates:cs.map(city=>({city,score:city.iso2===pick.iso2?1:0})),distanceKm:hav(cur,pick),latencyMs};
...
export default function Home(){const[cities,setCities]=useState<Capital[]>([]),[err,setErr]=useState(""),[result,setResult]=useState<Result|null>(null),[bench,setBench]=useState<Result[]>([]),[step,setStep]=useState(0),[speed,setSpeed]=useState(1),[algorithm,setAlgorithm]=useState("JEV + 2-opt"),[region,setRegion]=useState("All"),[showCandidates,setShowCandidates]=useState(true),[running,setRunning]=useState(false),[providerMode,setProviderMode]=useState<"demo"|"live">("demo"),[aiConfigured,setAiConfigured]=useState(false),[aiModel,setAiModel]=useState(AI_MODELS[0]),[copilotConfigured,setCopilotConfigured]=useState(false),[copilotModel,setCopilotModel]=useState("gpt-5.4"),[benchModels,setBenchModels]=useState<string[]>([AI_MODELS[0]]),[benchCopilotModels,setBenchCopilotModels]=useState<string[]>(["gpt-5.4"]),[benchAlgos,setBenchAlgos]=useState<string[]>([...BASE_ALGOS]),[benchLive,setBenchLive]=useState<{algorithm:string;route:Capital[];decisions:Decision[]}|null>(null),[textMode,setTextMode]=useState(false),[log,setLog]=useState<LogEntry[]>([]),[startIso,setStartIso]=useState("HOME"),[settings,setSettings]=useState<Settings>(EMPTY_SETTINGS),[settingsDraft,setSettingsDraft]=useState<Settings>(EMPTY_SETTINGS),[settingsSaved,setSettingsSaved]=useState(false),busyRef=useRef(false),logEndRef=useRef<HTMLDivElement>(null),routeEndRef=useRef<HTMLDivElement>(null);
function checkProviders(s:Settings){fetch("/api/jev",{headers:jevHeaders()}).then(r=>r.json()).then(d=>setProviderMode(d.configured?"live":"demo")).catch(()=>{});fetch("/api/ai-engine",{headers:aiHeaders()}).then(r=>r.json()).then(d=>setAiConfigured(!!d.configured)).catch(()=>{});fetch("/api/copilot",{headers:copilotHeaders()}).then(r=>r.json()).then(d=>{setCopilotConfigured(!!d.configured);if(d.model)setCopilotModel(d.model);if(Array.isArray(d.models)&&d.models.length)setBenchCopilotModels(d.models)}).catch(()=>{})}
...
async function run(){if(!cities.length||busyRef.current)return;busyRef.current=true;setRunning(true);setLog([]);const t=performance.now();
  try{
    const isJevLike=algorithm.startsWith("JEV"),isAiLike=algorithm==="AI Engine"||algorithm==="AI Engine + 2-opt",isCopilotLike=algorithm==="Copilot"||algorithm==="Copilot + 2-opt";
