...
const BASE_ALGOS=["Random","Nearest Neighbor","NN + 2-opt","JEV Demo","JEV Demo + 2-opt"];
...
async function run(){if(!cities.length||busyRef.current)return;busyRef.current=true;setRunning(true);setLog([]);const t=performance.now();
  try{
    const isJevLike=algorithm==="JEV Demo"||algorithm==="JEV Demo + 2-opt",isAiLike=algorithm==="AI Engine"||algorithm==="AI Engine + 2-opt";
    if(isJevLike||isAiLike){
      ...
      const onStep=(d:Decision,routeSoFar:Capital[])=>{
        ...
        pushLog({step:d.step,from:d.from.capital,to:d.selected.capital,km:d.distanceKm,pct:Math.round(d.confidence*100),candidates:d.candidates.map(x=>x.city.capital).join(", ")});
      };
...
async function benchmark(){if(!cities.length||busyRef.current)return;busyRef.current=true;setRunning(true);setBench([]);try{
  ...
  if(benchAlgos.includes("Nearest Neighbor")||benchAlgos.includes("NN + 2-opt")){
    const n=nearest(tourCities,startCity);
    if(benchAlgos.includes("Nearest Neighbor"))push(await t(async()=>({algorithm:"Nearest Neighbor",route:n,distanceKm:dist(n),runtimeMs:0,decisions:[]})));
    if(benchAlgos.includes("NN + 2-opt"))push(await t(async()=>{const x=twoOpt(n);return{algorithm:"NN + 2-opt",route:x,distanceKm:dist(x),runtimeMs:0,decisions:[]}}));
  }

  if(benchAlgos.includes("JEV Demo")||benchAlgos.includes("JEV Demo + 2-opt")){
    setBenchLive({algorithm:"JEV Demo",route:[startCity],decisions:[]});
    ...
    setBenchLive(null);
    if(benchAlgos.includes("JEV Demo"))push(await t(async()=>({algorithm:"JEV Demo",route:j.route,distanceKm:dist(j.route),runtimeMs:0,decisions:j.decisions})));
    if(benchAlgos.includes("JEV Demo + 2-opt"))push(await t(async()=>{const x=twoOpt(j.route);return{algorithm:"JEV Demo + 2-opt",route:x,distanceKm:dist(x),runtimeMs:0,decisions:j.decisions}}));
  }
...
<section className="controls"><button onClick={run} disabled={!cities.length||running}>{running?"RUNNING":"RUN"}</button><button className="ghost" onClick={benchmark} disabled={!cities.length||running}>BENCHMARK ALL</button><label>START<select value={startIso} onChange={e=>setStartIso(e.target.value)} disabled={running}><option value="HOME">Bengaluru (home base)</option>{startOptions.map(c=><option key={c.iso2} value={c.iso2}>{c.capital} — {c.country}</option>)}</select></label><details className="modelPicker" onToggle={e=>{if((e.target as HTMLDetailsElement).open)setSettingsDraft(settings)}}><summary>SETTINGS</summary><div className="modelPickerBody settingsBody">
  <b className="pickerGroup">JEV PROVIDER</b>
  <label className="settingsField">API URL<input type="text" placeholder="https://your-jev-endpoint.example/decide" value={settingsDraft.jevApiUrl} onChange={e=>setSettingsDraft({...settingsDraft,jevApiUrl:e.target.value})}/></label>
  <label className="settingsField">API Key<input type="password" placeholder="sk-…" value={settingsDraft.jevApiKey} onChange={e=>setSettingsDraft({...settingsDraft,jevApiKey:e.target.value})}/></label>
  <label className="settingsField">Model <small>(optional, defaults to jev-latest)</small><input type="text" placeholder="jev-latest" value={settingsDraft.jevModel} onChange={e=>setSettingsDraft({...settingsDraft,jevModel:e.target.value})}/></label>
  <b className="pickerGroup">OPENROUTER (AI ENGINE)</b>
  <label className="settingsField">API Key<input type="password" placeholder="sk-or-…" value={settingsDraft.openrouterApiKey} onChange={e=>setSettingsDraft({...settingsDraft,openrouterApiKey:e.target.value})}/></label>
  <small className="aiHint">Stored only in this browser (localStorage) and sent only to this app's own API routes — never anywhere else.</small>
  <button type="button" className={settingsSaved?"savedBtn":""} onClick={()=>saveSettings(settingsDraft)}>{settingsSaved?"SAVED ✓":"SAVE"}</button>
</div></details><details className="modelPicker"><summary>SELECT ({benchAlgos.length+benchModels.length})</summary><div className="modelPickerBody"><b className="pickerGroup">ALGORITHMS</b>{BASE_ALGOS.map(a=><label key={a}><input type="checkbox" checked={benchAlgos.includes(a)} onChange={e=>setBenchAlgos(prev=>e.target.checked?[...prev,a]:prev.filter(x=>x!==a))}/> {a}</label>)}<b className="pickerGroup">AI MODELS{!aiConfigured&&" (no key)"}</b>{AI_MODELS.map(m=><label key={m}><input type="checkbox" checked={benchModels.includes(m)} disabled={!aiConfigured} onChange={e=>setBenchModels(prev=>e.target.checked?[...prev,m]:prev.filter(x=>x!==m))}/> {m}</label>)}</div></details><label>ALGORITHM<select value={algorithm} onChange={e=>setAlgorithm(e.target.value)}><option>JEV Demo + 2-opt</option><option>JEV Demo</option><option>NN + 2-opt</option><option>Nearest Neighbor</option><option>Random</option><option disabled={!aiConfigured}>AI Engine + 2-opt</option><option disabled={!aiConfigured}>AI Engine</option></select></label>{!aiConfigured&&<small className="aiHint">Set OPENROUTER_API_KEY in .env.local to enable AI Engine</small>}{(algorithm==="AI Engine"||algorithm==="AI Engine + 2-opt")&&<label>MODEL<select value={aiModel} onChange={e=>setAiModel(e.target.value)}>{AI_MODELS.map(m=><option key={m} value={m}>{m}</option>)}</select></label>}<label>SPEED<select value={speed} onChange={e=>setSpeed(+e.target.value)}><option value="0.5">0.5×</option><option value="1">1×</option><option value="2">2×</option><option value="4">4×</option></select></label><label>MODE<div className="viewToggle"><button type="button" className={!textMode?"active":""} onClick={()=>setTextMode(false)}>MAP</button><button type="button" className={textMode?"active":""} onClick={()=>setTextMode(true)}>TEXT</button></div></label></section>
