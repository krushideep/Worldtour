...
function policyScore(city:Capital,confidence:number,strategy:string,f:{currentKm:number;nearestFutureKm:number;farthestFutureKm:number},current:Capital){const near=1/(1+f.nearestFutureKm/3000),far=1/(1+f.farthestFutureKm/10000),local=1/(1+f.currentKm/5000),same=city.region===current.region?1:0;if(strategy==="isolate")return confidence*.72+far*.16+local*.08+(1-same)*.04;if(strategy==="cluster")return confidence*.72+near*.16+local*.08+same*.04;return confidence*.72+local*.14+near*.07+far*.07}
...
async function jevDecide(step:number,current:Capital,cs:Capital[],rem:Capital[],settings:Settings,mode:JEVMode,matrix:Record<string,Record<string,number>>):Promise<{ranked:{city:Capital;confidence:number}[];mode:"demo"|"live";latencyMs?:number;strategy?:string}>{
  ...
  const featureByIso=new Map(features.map(f=>[f.iso2,f]));
  const strategy=String(data.strategy||"bridge");
  const ranked=[...raw].sort((a,b)=>policyScore(b.city,b.confidence,strategy,featureByIso.get(b.city.iso2)!,current)-policyScore(a.city,a.confidence,strategy,featureByIso.get(a.city.iso2)!,current));
