...
async function claudeCliDecide(step:number,current:Capital,cs:Capital[],rem:Capital[],model:string):Promise<{selected:Capital;latencyMs:number}>{
  const started=performance.now();
...
async function claudeCliEngine(c:Capital[],start:Capital,model:string,onStep?:(d:Decision,routeSoFar:Capital[])=>void){
  ...
  while(rem.length){
    ...
    const cs=candidates(cur,rem,route.length*19);
    const{selected,latencyMs}=await claudeCliDecide(route.length,cur,cs,rem,model);
    const pick=selected;
    const d:Decision={step:route.length,from:cur,selected:pick,candidates:cs.map(city=>({city,score:city.iso2===pick.iso2?1:0})),distanceKm:hav(cur,pick),latencyMs};
    ds.push(d);route.push(pick);onStep?.(d,[...route]);rem=rem.filter(x=>x.iso2!==pick.iso2);cur=pick;
  }
...
async function run(){if(!cities.length||busyRef.current)return;busyRef.current=true;setRunning(true);setLog([]);const t=performance.now();
  try{
    const isJevLike=algorithm.startsWith("JEV"),isAiLike=algorithm==="AI Engine"||algorithm==="AI Engine + 2-opt",isCopilotLike=algorithm==="Copilot"||algorithm==="Copilot + 2-opt",isClaudeCliLike=algorithm==="Claude CLI"||algorithm==="Claude CLI + 2-opt";
    if(isJevLike||isAiLike||isCopilotLike||isClaudeCliLike){
